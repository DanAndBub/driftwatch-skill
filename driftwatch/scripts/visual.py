...
document.getElementById('meta').innerHTML =
  `${{data.workspace}} &middot; ${{data.timestamp}} &middot; v${{data.version}}`;
// Summary
...
document.getElementById('budget-bars').innerHTML = files.map((f,i) => {{
  ...
  if(sim && sim.simulation_needed) {{
    ...
    detail += `<div class="danger-zone">⚠️ ${{status}}: ${{sim.recommendation||''}}</div>`;
  }}
  if(trend) {{
    const dtl = trend.days_to_limit;
    detail += `<div>Growth: ${{trend.growth_rate_chars_per_day}} chars/day` +
      (dtl!==null?` &middot; ${{dtl}} days to limit`:'') +
      ` <span class="trend-tag ${{trend.trend}}">${{trend.trend}}</span></div>`;
  }}
...
if(simFiles.length) {{
  ...
  document.getElementById('simulation-content').innerHTML = simFiles.map(f=>
    `<div class="finding"><span class="severity ${{f.status==='truncated_now'?'critical':'warning'}}">${{f.status==='truncated_now'?'TRUNCATED':'AT RISK'}}</span> ${{f.file}}: ${{f.recommendation||''}}</div>`
  ).join('');
}}
// Trends
if(data.trends?.files) {{
  ...
  document.getElementById('trends-content').innerHTML =
    ...
    data.trends.files.map(f=>
      `<div class="finding">${{f.file}}: ${{f.growth_rate_chars_per_day}} chars/day <span class="trend-tag ${{f.trend}}">${{f.trend}}</span>` +
      (f.days_to_limit!==null?` &middot; ${{f.days_to_limit}} days to limit`:'') + `</div>`
    ).join('');
}}
// Compaction
const anchors = data.compaction?.anchor_sections||[];
document.getElementById('compaction-content').innerHTML = anchors.map(a=>
  `<div class="anchor-row"><span>${{a.heading}} ${{a.found?'✓':'✗'}}</span><span>${{a.found?(a.char_count||0).toLocaleString()+' / 3,000 chars':'Missing!'}}</span></div>`
).join('');
// Hygiene
const findings = data.hygiene?.findings||[];
document.getElementById('hygiene-content').innerHTML = findings.length?
  findings.map(f=>`<div class="finding"><span class="severity ${{f.severity}}">${{f.severity}}</span> ${{f.message}}</div>`).join('') :
